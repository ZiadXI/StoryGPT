...
```bash
git clone https://github.com/ZiadXI/StoryGPT
cd StoryGPT
pip install -r StoryGPT/requirements.txt
```

Place `best_model.pt` in the project root, then run:

```bash
python StoryGPT/generate.py
```
...
│   ├── notebooks/
│   │   └── StoryGPT.ipynb    # Self-contained Kaggle training notebook
│   └── tokenizer/
│       └── storygpt_tokenizer/
